...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(dataloader):
        ...
        # Real images
        real_feature, real_validity = discriminator(real_imgs)
        # Fake images
        fake_feature, fake_validity = discriminator(fake_imgs)
        # Gradient penalty
        ...
        if i % opt.n_critic == 0:
            ...
            fake_feature, fake_validity = discriminator(fake_imgs)
            #pdb.set_trace()
            loss = feature_loss(F.adaptive_avg_pool2d(fake_feature, 1).squeeze(), F.adaptive_avg_pool2d(real_feature, 1).squeeze())
            g_loss = -torch.mean(fake_validity) + loss
            ...
            if batches_done % opt.sample_interval == 0:
                save_image(fake_imgs.data[:25], PATH + "%d.png" % batches_done, nrow=5, normalize=True)
